...
const FAQ_SCHEMA = {
  ...
  mainEntity: [
    ...
    },
    {
      ...
      acceptedAnswer: {
        '@type': 'Answer',
        text: 'It is declared closed and is not enforced. The Crossref XSD defines update-type as a closed enumeration of 12 values on a required attribute. The live index held 34 distinct values on 16 August 2026, of which 22 fall outside that enumeration, across 365 records. Examples include retration and retracion, two different misspellings of retraction; Retraction and Erratum as case variants, invalid because the type is case-sensitive; expression-of-concern hyphenated alongside expression_of_concern; a bare integer 68818; and test strings such as this_is_some_update_23 sitting in the production index. The point is not prevalence, since 365 records is 0.077 per cent, but governance: a closed enumeration on a required attribute is not being enforced at deposit time. The misspelling retration belongs to DOI 10.1016/j.cie.2010.04.003, published by Elsevier, whose title begins with the word RETRACTED.',
      },
    },
  ],
};
...
export const ScholarlyRecordOntology: React.FC = () => (
  <article className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-12">
    ...
    <section className="space-y-4">
      ...
      <p className="text-gov-dark leading-relaxed">
        Crossref&apos;s update-type field is how the scholarly record states what kind of correction has occurred. The schema (<code>common5.4.0.xsd</code>) declares it a <strong>closed enumeration of 12 values on a required attribute</strong>. The live index held <strong>34 distinct values</strong> on 16 August 2026, of which <strong>22 fall outside that enumeration</strong>, across 365 records. Among them: <code>retration</code>, a misspelling of retraction; <code>retracion</code>, a different misspelling; <code>Retraction</code> and <code>Erratum</code>, case variants that are invalid because the type is case-sensitive; <code>expression-of-concern</code> hyphenated alongside <code>expression_of_concern</code>; <code>68818</code>, a bare integer; and <code>this_is_some_update_23</code>, which is test data sitting in the production index.
      </p>
      <p className="text-gov-dark leading-relaxed">
        The argument here is not about prevalence. 365 records out of 475,593 is 0.077 per cent, and anyone who objects &quot;so what&quot; to that number is right to. The finding is about governance: the schema declares a closed list on a required attribute, and a string reading <code>this_is_some_update_23</code> is in the production index regardless. Validation is not enforced at deposit time on this path.
      </p>
      <p className="text-sm text-gov-secondary/90 leading-relaxed">
        An earlier version of this page reported 19 distinct values. That figure was wrong and is withdrawn. Our pipeline harvests by querying 13 named update-types, so it could only ever observe values co-occurring on records those queries returned; strings like <code>this_is_some_update_23</code> were unreachable. Crossref&apos;s facets enumerate values more completely than a seeded harvest but undercount volumes badly, and the two need to be used for opposite purposes. The correction is recorded in the repository&apos;s build report rather than quietly applied.
      </p>
      <p className="text-gov-dark leading-relaxed">
    ...
    <section className="space-y-4">
      ...
      <p className="text-gov-dark leading-relaxed">
        There is also a hard logical defect. In FaBiO version 2.3, <code>fabio:hasRetractionDate</code> is declared an <code>owl:FunctionalProperty</code>, while <code>fabio:hasCorrectionDate</code> is not. A functional property may take only one value per subject. So two registers reporting different retraction dates for the same paper does not merely go unmodelled: it makes the graph <strong>logically inconsistent</strong>. In the incumbent vocabulary, register disagreement about a retraction date is a contradiction rather than an observation, and the defect is specific to retraction.
      </p>
      <p className="text-gov-dark leading-relaxed">
    ...
    <section className="space-y-4">
      ...
      <p className="text-gov-dark leading-relaxed">
        One thing needs saying plainly before anything else in this section. <strong>The fact that retraction registers disagree is not a discovery of this study.</strong> It is an established research programme, principally that of Jodi Schneider and colleagues. Salami, McCumber and Schneider (2026) compare <strong>eleven</strong> sources across a union of 83,317 items and find that 92.64 per cent carry some disagreement, with only 41 items recorded as retracted in all eleven; their earlier four-source study found 3 per cent agreement. The 72.42 per cent figure reported above is a two-source special case of that result. Anyone citing this work should cite theirs.
      </p>
      <p className="text-gov-dark leading-relaxed">
